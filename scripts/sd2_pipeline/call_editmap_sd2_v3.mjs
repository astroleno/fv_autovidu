#!/usr/bin/env node
/**
 * EditMap-SD2 v3：输出 `{ markdown_body, appendix }`（见 1_EditMap-SD2-v3.md）。
 */
import fs from 'fs';
...
import { getEditMapSd2V3PromptPath } from './lib/sd2_prompt_paths_v3.mjs';
...
async function main() {
  ...
  console.log(
    `[call_editmap_sd2_v3] 调用 LLM：model=${getResolvedLlmModel()} base=${getResolvedLlmBaseUrl()}`,
  );
  console.log('[call_editmap_sd2_v3] 生成 EditMap-SD2 v3 …');
  ...
  try {
    ...
    console.error('[call_editmap_sd2_v3] JSON 解析失败，原始前 500 字：');
  ...
  console.log(`[call_editmap_sd2_v3] 已写入 ${outPath}`);
}

const _e = process.argv[1];
if (_e && pathToFileURL(path.resolve(_e)).href === import.meta.url) {
  main().catch((err) => {
    console.error('[call_editmap_sd2_v3]', err instanceof Error ? err.message : err);
    process.exit(1);
  });
}
