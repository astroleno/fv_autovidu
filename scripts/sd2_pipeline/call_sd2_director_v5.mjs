#!/usr/bin/env node
/**
 * SD2 v5：读取 sd2_director_payloads.json（kind: sd2_director_payloads_v5），按 Block 调用 LLM
 * （2_SD2Director-v5.md）。payload 含 knowledgeSlices 时拼入 system，user JSON 省略。
 *
 * 与 v4 的差异仅在：默认系统提示词路径（v5）。切片路由已由上游 build 阶段写好在 payload 内，
 * 本脚本不负责重新路由。若需要完整切片 + prevBlockContext + 路由审计，请使用
...
const DEFAULT_PROMPT = getDirectorSd2V5PromptPath();
...
async function main() {
  ...
  const promptPath =
    typeof args['prompt-file'] === 'string' && args['prompt-file'].trim()
      ...
      : DEFAULT_PROMPT;
  assertV5PromptFileExists(promptPath);
  ...
  console.log(
    `[call_sd2_director_v5] LLM：model=${getResolvedLlmModel()} base=${getResolvedLlmBaseUrl()} ` +
      `parallel=${useFullParallel ? 'full' : `pool(${concurrency})`} staggerMs=${staggerMs}`,
  );
  console.log(`[call_sd2_director_v5] 系统提示词: ${promptPath}`);
  ...
  async function processOne(entry, index) {
    ...
    const userMessage = [
      '以下为单 Block 的 SD2Director v5 输入 JSON（editMapParagraph、blockIndex、assetTagMapping、prevBlockContext、v5Meta 等；knowledgeSlices 已注入 system）。',
      '请严格按系统提示输出唯一一个 JSON 对象（含 markdown_body 与 appendix），不要 Markdown 围栏。',
      '',
      JSON.stringify(userObj, null, 2),
    ].join('\n');

    console.log(`[call_sd2_director_v5] Block ${blockId} …`);
  ...
  if (useFullParallel) {
    const rows = await Promise.all(list.map((entry, index) => processOne(entry, index)));
  ...
  console.log(`[call_sd2_director_v5] 单块目录: ${outDir}`);
  console.log(`[call_sd2_director_v5] 汇总: ${allPath}`);
}

const _e = process.argv[1];
if (_e && pathToFileURL(path.resolve(_e)).href === import.meta.url) {
  main().catch((err) => {
    console.error('[call_sd2_director_v5]', err instanceof Error ? err.message : err);
    process.exit(1);
  });
}
