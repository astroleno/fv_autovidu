#!/usr/bin/env node
/**
 * SD2 v4：读取 sd2_payloads.json（kind: sd2_prompter_payloads_v4），按 Block 调用 LLM（2_SD2Prompter-v4.md）。
 * payload 中的 knowledgeSlices 拼入 system prompt，user JSON 中省略（与 call_sd2_block_chain_v4 一致）。
 *
 * 用法:
 *   node scripts/sd2_pipeline/call_sd2_prompter_v4.mjs \
 *     --payloads output/sd2/{id}/sd2_payloads.json \
 *     --out-dir output/sd2/{id}/prompts \
 *     [--prompt-file prompt/.../2_SD2Prompter-v4.md] [--stagger-ms 400] [--concurrency 4]
 */
import fs from 'fs';
...
async function main() {
  ...
  console.log(
    `[call_sd2_prompter_v4] LLM：model=${getResolvedLlmModel()} base=${getResolvedLlmBaseUrl()} ` +
      `parallel=${useFullParallel ? 'full' : `pool(${concurrency})`} staggerMs=${staggerMs}`,
  );
  console.log(`[call_sd2_prompter_v4] 系统提示词: ${promptPath}`);
  ...
  async function processOne(entry, index) {
    ...
    console.log(`[call_sd2_prompter_v4] Block ${blockId} …`);
  ...
  console.log(`[call_sd2_prompter_v4] 单块文件目录: ${outDir}`);
  console.log(`[call_sd2_prompter_v4] 汇总: ${allPath}`);
}

const _e2 = process.argv[1];
if (_e2 && pathToFileURL(path.resolve(_e2)).href === import.meta.url) {
  main().catch((err) => {
    console.error('[call_sd2_prompter_v4]', err instanceof Error ? err.message : err);
    process.exit(1);
  });
}
