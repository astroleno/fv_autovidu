#!/usr/bin/env node
/**
 * SD2 v5：读取 sd2_payloads.json（kind: sd2_prompter_payloads_v5），按 Block 调用 LLM
 * （2_SD2Prompter-v5.md）。payload 中的 knowledgeSlices 拼入 system prompt，user JSON 省略。
 *
 * 与 v4 的差异仅在：默认系统提示词路径（v5）。切片路由由上游 build 阶段写好在 payload 内。
 */
import fs from 'fs';
...
async function main() {
  ...
  console.log(
    `[call_sd2_prompter_v5] LLM：model=${getResolvedLlmModel()} base=${getResolvedLlmBaseUrl()} ` +
      `parallel=${useFullParallel ? 'full' : `pool(${concurrency})`} staggerMs=${staggerMs}`,
  );
  console.log(`[call_sd2_prompter_v5] 系统提示词: ${promptPath}`);
  ...
  async function processOne(entry, index) {
    ...
    console.log(`[call_sd2_prompter_v5] Block ${blockId} …`);
  ...
  console.log(`[call_sd2_prompter_v5] 单块目录: ${outDir}`);
  console.log(`[call_sd2_prompter_v5] 汇总: ${allPath}`);
}

const _e = process.argv[1];
if (_e && pathToFileURL(path.resolve(_e)).href === import.meta.url) {
  main().catch((err) => {
    console.error('[call_sd2_prompter_v5]', err instanceof Error ? err.message : err);
    process.exit(1);
  });
}
