...
2. **末位位置硬约束（v5.0-rev1 · 新增）**：本块**最末 1 个时间片**（即 `shots[shots.length - 1]`）必须满足：
   - `shot_code` ∈ `[B1, B2]`（主角反应特写档位）；
   - `has_protagonist == true`（且画面中为主角本人，不是背影/剪影/画外音）；
   - 时长 ≥ 1.5s；
   - 情绪内容紧扣本块爽点兑现节拍（反杀 / 边界 / 偏爱 / 正义）。
   - **编排时必须先锁死末位时间片**，再回头编排前面其他时间片（"末位优先法"）；不得让反派胜利定格 / 反派甜言蜜语 / 空镜 / 环境镜占据末位。
3. `ratio_actual` 的**下限硬拉到 0.5**（覆盖 `payoff_block_min` 默认 0.60 中的 50% 兜底线）；若 EditMap 给的 `shot_ratio_target` < 0.5，以 0.5 为准。
4. 自估不达标 / 末位硬约束违反时：
   - 第一轮追加 1 个 `[B1]` 主角反应特写重估（若缺末位，直接把末位替换为主角反应）；
   - 仍不达标 → `check = false` 且在 `continuity_out.notes` 里写入：
     - 比例不达标 → `"payoff_without_protagonist_reaction: <reason>"`；
     - **末位位置硬约束违反** → `"payoff_tail_slot_not_protagonist: tail_shot_code=<code> tail_has_protagonist=<bool> reason=<why>"`。
...
| **末位给反派胜利定格 / 反派甜言蜜语 / 空镜 / 环境镜** | 最后画面 = 观众情感锚点，决定爽点主体 | 把反派得意放在倒数第二片，**末位严格留给主角反应特写** |

**自报字段**：若本块判定为 payoff block，在 `continuity_out.notes` 里追加两行：
- `"payoff_reaction_shots: [<shot_id_1>, <shot_id_2>...]"` 列出你用作主角反应特写的镜头 ID；
- `"payoff_tail_slot_ok: <bool>"`（`true` 表示末位硬约束满足；`false` 时必须同行附 `reason`）。
...
1. **若本块为 payoff block（见 §I.9.1 识别条件）**：先**锁死末位时间片为 `[B1/B2]` 主角反应特写**（时长 ≥ 1.5s），再回头编排前面时间片；这是"末位优先法"的硬约束（§I.9.1 硬约束 2），其他规则必须围绕这条倒推，不能反过来。
...
2. v5 部分：
   ...
   - **若本块为 payoff block**：检查末位时间片是否满足 §I.9.1 硬约束 2（`shot_code ∈ [B1,B2]` + `has_protagonist=true` + 时长 ≥ 1.5s），结果写入 `notes[]` 的 `payoff_tail_slot_ok: <bool>`；若为 `false` 必须同行附 `reason`；同时追加 `payoff_reaction_shots: [<ids>]`；
   - 任何 warning 记入 `notes[]`。
...
19. **payoff block 末位位置硬约束**（§I.9.1 硬约束 2）：若本块为 payoff block，`shots[last].shot_code ∈ [B1,B2]`、`has_protagonist == true`、时长 ≥ 1.5s；违反时必须在 `notes[]` 写入 `payoff_tail_slot_not_protagonist: ...` 与 `payoff_tail_slot_ok: false` 并附 `reason`。
