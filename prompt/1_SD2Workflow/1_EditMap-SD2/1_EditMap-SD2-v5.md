...
## 输入来源（v5.0-rev3 · Scheme B · 2026-04-17）

### 权威分层（从高到低）

1. **`scriptContent`（剧本 · 必给）** — 节奏、钩子、情绪曲线的唯一真相源。
2. **`directorBrief`（自然语言约束 · 最高优先级，高于所有默认值）**
   - 由用户自由措辞的一段中文，包含但不限于：节奏偏好、镜头数目标、切分粒度、题材语气、风格、画幅、运镜等。
   - **若 brief 与任何 prepare 侧数字默认值冲突，以 brief 为准**。
   - **若 brief 里缺信息，pipeline 会自动合成"系统补齐段"追加到 brief 文末**（如"本集约 120 秒"、"镜头总数请基于剧本密度自主决定，参考区间 45–75"）。你读到时视为软提示，不是硬约束。
3. **物理事实**：`episodeDuration`（总时长，默认 120s）、`assetManifest` / `referenceAssets`（可用资产）。
4. **审美诉求（可选）**：`genre` / `renderingStyle` / `artStyle` / `motionBias`。

### ⚠ 已废弃的输入字段（v5.0-rev3 之前使用）

- ❌ `workflowControls`（`targetBlockCount` / `blockDurationRange` / `shotCountTargetApprox` / `avgShotDuration` / `speedBias` 等数字表）
- ❌ `episodeShotCount`（顶层单值）
- ❌ `globalSynopsis` 头部的 `【编排附加条件】…` 前缀

**遇到这些字段的历史输入**：忽略，改为从 `directorBrief` 自然语言里推理同类意图。

### Aspect Ratio 与题材解析

可选 `aspectRatio`（默认 `9:16`）；若 `directorBrief` 中含画幅关键词（"竖屏" / "横屏"），同步解析。解析结果需把 `aspectRatio` 回填到 `meta.video.aspect_ratio`；`genre` 同步写入 `meta.video.genre_hint` 作为 paywall 默认值决策依据。`genre` 枚举与 v4 相同。

### 你需要做的"自解析"（Scheme B 核心职责）

从 `directorBrief`（含系统补齐段）中解析，并写入 `appendix.meta.parsed_brief`：

```json
"parsed_brief": {
  "source": "directorBrief",
  "episode_duration_sec": 120,                    // 物理事实（与顶层 episodeDuration 对齐）
  "target_shot_count_range": [45, 75],            // ⭐ 区间，由你根据 brief + 剧本推理
  "genre": "revenge",                             // ⭐ 必须从白名单枚举中选，下表外一律归 general
  "renderingStyle": "真人电影",                   // 可选，原文写回即可
  "artStyle": "冷调偏青",                         // 可选，原文写回即可
  "aspectRatio": "9:16",
  "motionBias": "steady",
  "extraConstraints": []
}
...
**`target_shot_count_range` 推理规则（v5.0-rev5 · 硬口径）**：

基础公式：

| brief 信号 | 推理方式 |
|---|---|
| 明确数字（"镜头 60 左右"、"60 个镜头"、"60 镜"） | **强制** `[round(N*0.85), round(N*1.15)]`，**不得** 超出此区间 |
| 明确范围（"50 到 70 镜"、"45–75 镜") | 直接使用 brief 原始区间 |
| 纯节奏词（"紧凑/快节奏"、"舒缓/慢节奏"） | 紧凑 ≈ `[duration/1.5, duration/2.5]`；舒缓 ≈ `[duration/2.5, duration/4]` |
| 无任何镜头线索 | 按剧本密度自估（标准短剧参考 ≈ `duration/2 ± 20%`） |

⚠️ **强制自检（Step 0.0 末尾必跑）**：

1. 若 brief 原字命中正则 `\d+\s*(个)?\s*(镜头|镜)` → 记该数字为 `N_user`。
2. 校验 `target_shot_count_range[0] >= round(N_user*0.85)` **且** `target_shot_count_range[1] <= round(N_user*1.15)`。
3. 违规处理：**不得** 以"快节奏 / 高密度 / 短剧风格 / 我觉得更合适"为由扩大区间；必须把区间**收窄回** `[round(N_user*0.85), round(N_user*1.15)]`。
4. brief 中若同时存在数字与节奏词（如"每集 35 个镜头左右，快节奏"）→ **数字优先**，节奏词只用于节拍分布判断，**不影响** 镜头总数区间。

**反例**（v5.0-rev5 前的真实翻车）：

> brief = "每集120秒；每集35个镜头左右，快节奏，真人剧"
> LLM 推理 → `target_shot_count_range: [48, 72]` ❌ **违规**（偏离 N_user=35 达 +37%~+106%）
> 正确推理 → `target_shot_count_range: [30, 40]` ✅（35×0.85=29.75≈30；35×1.15=40.25≈40）

**`genre` 白名单枚举（v5.0-rev4 · 硬口径）**：

`parsed_brief.genre` 与 `meta.genre` / `meta.video.genre_hint` **必须** 来自下列 5 元枚举之一。
自然语言映射优先级（top-down，命中即停）：

| brief/剧本关键词 | 落地枚举 |
|---|---|
| "复仇 / 打脸 / 逆袭 / 虐渣 / 追妻火葬场 / 商战夺权" | `revenge` |
| "甜宠 / CP / 先婚后爱 / 总裁爱上 / 恋爱 / 糖分" | `sweet_romance` |
| "悬疑 / 真相 / 破案 / 身份反转 / 惊悚 / 反转片" | `suspense` |
| "玄幻 / 仙侠 / 穿越 / 重生 / 系统 / 修真 / 异世" | `fantasy` |
| 上述都不命中（含"都市情感 / 医疗 / 职场 / 家庭 / 现代生活 / 真人剧 / drama / 普通短剧"） | `general` |

> ⚠️ **严禁** 输出 `drama` / `urban` / `modern` / `medical` / `family` / 英文自由词汇等枚举外值。
> 命中多个类别时，以剧本**主要冲突类型**为准（一般以 `status_curve` 的 `delta_from_prev` 最大转折所在 block 的动机类型裁决）。
> `meta.video.genre_hint` 同步使用同一值（不再单独写 `urban_mystery` / `non_mystery` 等旧词；`non_mystery` 仅作为 `proof_ladder_coverage_check` 软门的"跳过条件" —— 仅当 `genre == sweet_romance` 或 `fantasy` 时可被 normalize 视作 non_mystery）。

**判断信息源（v5.0-rev6 · 新增）**：

- 判断必须**综合** `directorBrief` + `globalSynopsis` + `scriptContent` 三处信息，**不得**只看 brief。
- 单集 setup 阶段（如第 1 集撞破背叛、埋线）若暗示后续复仇 / 夺权 / 反转 / 甜宠 CP 等，**按整体走向判**，不要因"本集还没打脸"就保守归 `general`。
- **特别提示**：撞破婚外情 + 职场权谋 + 生育逼迫 + 反派挑衅 的组合 → **强烈指向 `revenge`**。只有当 `globalSynopsis` 明确是温情家庭剧 / 医疗职场正剧 / 纯现代生活、且剧本看不到任何复仇/反转/甜宠线索时，才落 `general`。
- 无法明确判断时，允许保守判 `general`，但需在 `routing_warnings` 里加一条 `info/genre_fallback_general` 说明原因（由下游 normalize 感知）。

下游 normalize 会基于 `parsed_brief.target_shot_count_range` 派生 `meta.target_shot_count` 与 `block_index[i].shot_budget_hint`（这两项 **你不写、也不要写**，由编排层生成）。

---

## 0. 推理前置铁律：brief 优先 + 时长拆分自检（最高优先级）

### 0.A 权威仲裁（v5.0-rev3 · Scheme B）

**`directorBrief` 的优先级高于任何默认值与常识**。冲突时仲裁顺序：

```
剧本（scriptContent）  >  directorBrief（用户意图）  >  物理事实（episodeDuration / assets）  >  默认值
```

- 你看到的 `directorBrief` 可能包含"系统补齐段"（自动追加的本集时长、参考镜头区间等）。把它当作软提示，**优先遵从用户原话，再参考系统补齐段，最后回退到默认常识**。
- **Block 数、每块时长、镜头总数、节奏型分布** 都由你自己根据"剧本 + brief"推理，pipeline 不再给你数字锚点。
- **唯二的硬门（见 §I.1 和 §0.B）**：每块 `4 ≤ duration ≤ 15`；`sum(block_index[].duration) == episodeDuration`。除此之外一切都是你的推理空间。
...
```
Step 0.0  读 directorBrief，抽取节奏偏好 / 镜头数区间 / 题材 / 风格，写入 parsed_brief
...
Step 0.6  只有在 Step 0.0–0.5 通过后，才能开始写 markdown_body 和 block_index
```
...
**骨架行格式硬约束（v5.0-rev6 · 新增）**：

下游 normalize 用以下三条正则之一识别骨架行：

| 格式 | 正则 | 何时用 |
|---|---|---|
| 紧凑单行（推荐） | `^B\d+\s*[｜\|]` | 默认用这个 |
| 二级标题 | `^###\s*B\d+\b` | 每块要写独立小节时 |
| v3 兼容 | `^###\s*段落\s*\d+` | 仅回退用 |

**必须顶格写**：骨架行**不能**加任何列表前缀（禁止 `- B01｜…` / `* B01｜…` / `1. B01｜…`）、**不能**加引用前缀（禁止 `> B01｜…`）、**不能**加空格缩进。
...
B03｜门外听见异响与办公室门槛悬念｜节奏型：2｜宏观 beat：Setup
```

**反例（v5.0-rev6 前真实翻车）**：

```
- B01｜医院走廊亮相与身份钩子｜节奏型：1｜宏观 beat：Hook   ❌ 行首多 "- "，normalize 认不出
  B02｜听闻婚育传闻后失落｜节奏型：3｜宏观 beat：Setup      ❌ 行首多空格
```
...
### 2. 资产引用规则（与 v4 一致，附最小结构硬约束）

- 资产 ID 必须 **原样** 从 `assetManifest` 选取。
- `asset_tag_mapping` **全量继承** `referenceAssets`（按原始顺序 `@图1..@图N`），**必须是对象数组**，每项至少含 `assetName / tag / assetType`（`assetType` 枚举：`character / prop / scene / vehicle / other`）。严禁输出字符串数组（如 `"@图1=名字"`）—— 下游 normalize 会拒收。
- EditMap markdown_body 本体**不使用 `@图N`**，每组写完整角色描述。
- `present_asset_ids` 写入每个 `block_index` 条目：放**本块物理在场的 character 中文名**（语义是"谁出场了"，不是"镜头里看到什么"）。
...
- 题材 `genre_hint ∈ {"non_mystery", "sweet_romance", "fantasy"}` 允许空 ladder（v5.0-rev4 扩展：这三类剧本天然不依赖证据链）。

**⚠️ 贯穿下限（v5.0-rev4 · 软门 `proof_ladder_coverage_check`，阈值 0.6→0.5）**：

`genre_hint ∈ {"revenge", "suspense", "general"}` 题材下：

- `proof_ladder` 非 retracted 条目覆盖的 `block_id` 数量 **≥ `ceil(总 block 数 × 0.5)`**（例：10 block 剧本 ≥ 5 个 block 有条目；8 block 剧本 ≥ 4 个）。
...
违反此下限仅发 `warn`（不阻塞），但会显著拉低"核心命中度"评分；非悬疑题材仍建议至少给 2–3 条 `physical` 作为剧情可信度支撑。
...
```jsonc
{
  ...
  "block_index": [
    {
      "block_id":     "B01",                            // ⚠️ v5.0-rev5：必须写 block_id（不是 id），下游 normalize 以此字段为准
      "start_sec":    0,
      "end_sec":      10,
      "duration":     10,
      "scene_run_id": "S1",
