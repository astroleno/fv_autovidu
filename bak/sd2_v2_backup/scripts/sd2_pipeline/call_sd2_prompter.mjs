#!/usr/bin/env node
/**
 * 读取 sd2_payloads.json，按 Block 调用 LLM（默认系统提示词 2_SD2Prompter-v2.md），生成 Seedance 三段式输出。
 *
 * 用法:
 *   node scripts/sd2_pipeline/call_sd2_prompter.mjs \
 *     --payloads output/sd2/{id}/sd2_payloads.json \
 *     --out-dir output/sd2/{id}/prompts \
 *     [--stagger-ms 400] [--concurrency 4]
 *
 * 默认：全 Block 并行（concurrency=Block 数），仅「启动时间」按序号错峰（stagger-ms，默认 400ms），
 * 降低瞬时打满 API。若需限并发可显式传 --concurrency N（N<总数 时走池化，无错峰）。
 */
import fs from 'fs';
...
async function main() {
  ...
  const promptPath =
    typeof args['prompt-file'] === 'string' && args['prompt-file'].trim()
      ...
      : getResolvedPrompterPromptPath();
  assertPromptFileExists(promptPath);
  ...
  console.log(
    `[call_sd2_prompter] LLM：model=${getResolvedLlmModel()} base=${getResolvedLlmBaseUrl()} ` +
      `parallel=${useFullParallel ? 'full' : `pool(${concurrency})`} staggerMs=${staggerMs}`,
  );
  console.log(`[call_sd2_prompter] 系统提示词: ${promptPath}`);
  ...
  async function processOne(entry, index) {
    ...
    const userMessage = [
      '以下为单 Block 的 SD2Prompter 输入 JSON（含 edit_map_block、asset_tag_mapping、prev_block_context、few_shot_context、rendering_style、art_style、aspect_ratio；若存在 sd2_director 则须与导演时间片一致）。',
      '请严格按系统提示输出唯一一个 JSON 对象，不要 Markdown 围栏。',
      '',
      JSON.stringify(payload, null, 2),
    ].join('\n');

    console.log(`[call_sd2_prompter] Block ${blockId} …`);
  ...
  console.log(`[call_sd2_prompter] 单块文件目录: ${outDir}`);
  console.log(`[call_sd2_prompter] 汇总: ${allPath}`);
}

const _e2 = process.argv[1];
if (_e2 && pathToFileURL(path.resolve(_e2)).href === import.meta.url) {
  main().catch((err) => {
    console.error('[call_sd2_prompter]', err instanceof Error ? err.message : err);
    process.exit(1);
  });
}
