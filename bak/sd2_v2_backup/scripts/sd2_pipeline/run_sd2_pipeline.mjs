...
async function main() {
  ...
  if (!episodeJson || !fs.existsSync(episodeJson)) {
    console.error(
      '请提供 --episode-json，或使用 --pull --project-id <uuid> --episode-id <uuid>',
    );
    process.exit(2);
  }
  ...
  if (!skipEditmap) {
    const editMapScript = useYunwu
      ? 'call_yunwu_editmap_sd2.mjs'
      : 'call_editmap_sd2.mjs';
    console.log(`[run_sd2_pipeline] LLM 生成 edit_map_sd2.json（${useYunwu ? 'Yunwu/Opus' : 'DashScope'}）…`);
    /** @type {string[]} */
    const emArgs = [
  ...
  if (useBlockChain) {
    ...
    /** @type {string[]} */
    const chainArgs = [
      path.join('scripts', 'sd2_pipeline', 'call_sd2_block_chain.mjs'),
      '--edit-map',
      editMapOut,
      '--director-payloads',
      directorPayloads,
    ...
    if (!dryRun && !skipDirector && skipPrompter) {
      ...
      runNode([
        path.join('scripts', 'sd2_pipeline', 'call_sd2_director.mjs'),
        '--payloads',
        directorPayloads,
        '--out-dir',
        directorPromptsDir,
  ...
  if (!useBlockChain) {
    ...
    /** @type {string[]} */
    const proArgs = [
      path.join('scripts', 'sd2_pipeline', 'call_sd2_prompter.mjs'),
      '--payloads',
      payloadsOut,
      '--out-dir',
      promptsDir,
