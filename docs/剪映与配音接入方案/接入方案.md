...
| 剪映字幕轨 | `subtitle_text_from_shot`：**优先** `dialogueTranslation`（与 Vidu/TTS 共用本机译文），否则 `dialogue`，再否则 `associatedDialogue`；时间轴与每段已选视频 `target_timerange` 对齐，轨道顺序为 video → text → audio |
| repull 合并 | `puller._LOCAL_SHOT_MERGE_KEYS` 含 `dialogue`、`dialogueTranslation`、`associatedDialogue`，重拉平台快照时不覆盖用户在本机编辑的台词与译文 |
