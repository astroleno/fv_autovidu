...
## 10. 按镜头关闭注入（`includeDialogueInVideoPrompt`）

| 项 | 说明 |
|----|------|
| **字段** | `Shot.includeDialogueInVideoPrompt`，默认 `true`；写入 `episode.json` |
| **为 `false` 时** | `_run_video_gen` 中 `composed_video_prompt = shot.videoPrompt`，**不**调用 `append_dialogue_for_video_prompt` |
| **不改** | `shot.videoPrompt` 落库；`dialogue` / `dialogueTranslation` 仍供字幕、配音、剪映等 |
| **UI** | 分镜表「译文」列后增加「视频含台词」列（`ShotDialogueInjectCell`），checkbox 即时 PATCH |
