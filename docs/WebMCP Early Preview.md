...
Form-associated elements support the `toolparamdescription` attribute, which provides an optional way to define a property description within the json-schema. In its absence, the browser uses the `textContent` of the associated `<label>` HTML element but skips descendants that are labelable or, if no label exists, the `aria-description`.
...
<script>
  document.querySelector("form").addEventListener("submit", (e) => {    e.preventDefault();
    if (!myFormIsValid()) {      if (e.agentInvoked) { e.respondWith(myFormValidationErrors) };      return;    }
