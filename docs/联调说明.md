...
**Python 环境**：仓库脚本**不创建、不使用**项目内 `.venv`。请在你要使用的解释器上安装依赖，例如：

```bash
pip install -r requirements.txt
```

使用 **conda** 时请先 `conda activate` 你的环境：`pnpm dev` 会优先使用 `$CONDA_PREFIX/bin/python`（避免 macOS 上 `python3` 仍指向 `/usr/bin` 而检测不到已安装的包）。若仍不对，可 `export FV_PYTHON=/你的/python` 再运行 `pnpm dev`。`postinstall` 用同一套规则检查能否 `import uvicorn`，不会建虚拟环境。
...
5. **单帧重生**（可选）：镜头详情或分镜卡片进入单帧重生页，修改画面描述并勾选资产（含剧集级资产库），需 `YUNWU_API_KEY`；前后端行为、任务轮询与资产解析见 [单帧重生.md](./单帧重生.md)
6. **选定**：在候选视频中选定一个
7. **导出**：导出粗剪视频（需 FFmpeg）；剪映草稿见下文「剪映导出与 reference 包」
...
**单副本保证**：同一 `episodeId` 在 `DATA_ROOT` 下只保留唯一一份目录。重新拉取时若 `projectId` 变化，puller 会自动迁移旧目录中的本地产物（视频/尾帧/配音/导出）到新路径并删除旧副本，不会出现多目录并存的情况。
